...
def create_spotlight_lab_table(new_table_name):
    ...
    table = dynamodb_client.create_table(
        ...
        BillingMode=
        'PAY_PER_REQUEST'  # Using the 'fire and forget' Pay Per Request pricing strategy
    )
    waiter = dynamodb_client.get_waiter('table_exists')
    waiter.wait(TableName=new_table_name
                )  # Waiting for the table to finish being created
...
with open('c:\\temp\\50000 Sales Records.csv', 'rt') as f:
    salesDataReader = csv.DictReader(f)

    itemCount = 0
    if dynamodb_resource.Table('Table1').scan(
    )['ScannedCount'] == 0:  #Wasteful action - don't do this in real life!.
        for salesRow in salesDataReader:
            if salesRow['Item Type'] == 'Office Supplies' and salesRow[
                    'Region'] == 'Europe':
...
with open('c:\\temp\\50000 Sales Records.csv', 'rt') as f:
    ...
    spotlight_lab_direct_kms_cmp = AwsKmsCryptographicMaterialsProvider(
        'arn:aws:kms:us-west-2:408344393370:key/6b361f68-cd6c-4494-bb0a-ae066462f2eb'
    )
    ...
    if dynamodb_resource.Table('Table2').scan(
    )['ScannedCount'] == 0:  #Wasteful action - don't do this in real life!.
        for salesRow in salesDataReader:
            if int(salesRow['Units Sold']
                   ) < 1000 and salesRow['Region'] == 'North America':
                print(salesRow)
                itemCount = itemCount + 1
...
with open('c:\\temp\\50000 Sales Records.csv', 'rt') as f:
    ...
    if dynamodb_resource.Table('Table3').scan(
    )['ScannedCount'] == 0:  #Wasteful action - don't do this in real life!.
        for salesRow in salesDataReader:
            if float(salesRow['Total Revenue']
                     ) < 500000 and salesRow['Item Type'] == 'Vegetables':
                print(salesRow)
                itemCount = itemCount + 1
                # Issue item storage request to EncryptedTable object
                encrypted_table_access.put_item(
...
recipes_directory = 'C:\\Personal\\Secret Family Recipes\\'
...
for recipe in recipe_file_list:
    ...
    keys = dict(key_ids=[
        'arn:aws:kms:us-west-2:408344393370:key/5255f291-a67a-4b47-9f2a-f93c902a9528'
    ])
    ...
    ciphertext, encryptor_header = client.encrypt(
        source=recipe_text, key_provider=master_key_provider)
